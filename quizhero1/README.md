...
## Backend options (added by ChatGPT)
This project now includes `/backend` with:
- `supabase/` → Postgres schema + setup
- `express/` → Local API server (Node.js)
- `pocketbase/` → Importable collections + setup
- `ai-generator/` → Prompt to auto-generate a backend project

See each folder README/SETUP for details.
