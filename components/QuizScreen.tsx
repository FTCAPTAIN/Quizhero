...
import { ExitIcon, LightBulbIcon } from './Icons';
    ...
    <>
      ...
      <div className="relative flex flex-col h-full w-full animate-fade-in">
        <header className="w-full md:hidden flex items-center justify-between h-16 px-4 shrink-0 bg-white dark:bg-gray-900 border-b border-slate-200 dark:border-gray-800">
          <button className="p-2 border border-slate-500 dark:border-gray-600 rounded-md text-slate-500 dark:text-gray-300" aria-label="Menu">
        ...
        <button onClick={handleExitClick} className="absolute top-20 md:top-6 left-6 text-slate-500 dark:text-gray-400 hover:text-slate-900 dark:hover:text-white transition-colors duration-200 z-10 p-2 rounded-full hover:bg-slate-200/50 dark:hover:bg-gray-700/50" aria-label="Exit Quiz">
          ...
          <div className="w-full max-w-3xl flex flex-col items-center">
            ...
            <div className="w-full max-w-md mb-4 px-4">
              <div className="flex justify-between items-center space-x-4">
                <div className="flex-grow bg-slate-200 dark:bg-gray-800 rounded-full h-1.5">
                  <div className="h-1.5 rounded-full transition-all duration-300" style={{ width: `${questionProgressPercentage}%`, backgroundColor: 'var(--accent-color)' }} />
                </div>
            ...
            {isAiEnabled && (
                <div className="w-full max-w-md mb-4 flex justify-end">
                    <button onClick={handleGetHint} disabled={hintUsed || selectedAnswer !== null || isHintLoading} className="flex items-center gap-2 text-sm bg-amber-500/10 text-amber-500 font-semibold py-2 px-4 rounded-full transition-colors hover:bg-amber-500/20 disabled:opacity-50 disabled:cursor-not-allowed">
